...
class DashboardSAMER:
    ...
    def load_data_to_table(self, table_name):
        ...
        try:
            ...
            # 2. Llamar a la función del módulo db_manager para obtener los datos
            filas, nombres_columnas = db_manager.cargar_datos_tabla(table_name)
            
            # 3. Configurar columnas del Treeview
            self.tree["columns"] = nombres_columnas
            self.tree["show"] = "headings"
            
            # 4. Configurar encabezados y anchos de columna
            for col in nombres_columnas:
                self.tree.heading(col, text=col)
                self.tree.column(col, width=120, anchor="center")
            
            # 5. Insertar datos en el Treeview
            for row in filas:
    ...
    def add_item(self):
        ...
        messagebox.showinfo(
            "Agregar", 
            f"Funcionalidad para agregar nuevo elemento en {section}"
        )
    ...
    def download_pdf(self):
        ...
        messagebox.showinfo(
            "Descargar Reporte", 
            f"Generando reporte PDF de {section}..."
        )
